...
def write_output_file(output_file_path, pa_df, de_df ,cd_df, pt_df, layout_dict, date_columns=[]):
    ...
    with open(output_file_path, 'w') as outfile:
        ...
        print("Writing PA data...")
        write_dataframe(pa_df, layout_dict['pa_layout'])
        
        # Write the synthetic bhd data
        print("Writing DET data...")
        write_dataframe(de_df, layout_dict['de_layout'])

        print("Writing CD data...")
        ...
        print("Writing PT data...")
        write_dataframe(pt_df, layout_dict['pt_layout'])
...
def main(use_same_metadata_version=True):
    ...
    pa_file_layout = file_layout_df[file_layout_df['Type'] == 'PA']
    de_file_layout = file_layout_df[file_layout_df['Type'] == 'DE']
    pt_file_layout = file_layout_df[file_layout_df['Type'] == 'PT']
    ...
    print("Processing CD records...")
    cd_rows = [{"CD_Record": line.strip()} for line in raw_df if line.startswith("CD")]
    cd_df = pd.DataFrame(cd_rows)

    raw_df = [line for line in raw_df if not line.startswith("CD")]
    ...
    start_time = time.time()
    date_columns = ['da']
    pa_df = process_file_data([raw_df[0]], pa_file_layout)
    de_df = process_file_data(raw_df[1:-1], de_file_layout, date_columns)
    pt_df = process_file_data([raw_df[-1]],   pt_file_layout)
    print(f"Time taken to process header and trailer: {time.time() - start_time:.2f} seconds")
    print('############################################################################')
    
  
    start_time = time.time()
    synthetic_pa_df, pa_metadata_df = generate_synthetic_data(
        pa_df, metadata_base_path, metadata_type="pa", use_same_metadata_version=use_same_metadata_version
    )
    print(f"Time taken to generate synthetic header data: {time.time() - start_time:.2f} seconds")
    print('############################################################################')

    start_time = time.time()
    synthetic_de_df, de_metadata_df = generate_synthetic_data(
        de_df, metadata_base_path, metadata_type="de", use_same_metadata_version=use_same_metadata_version
    )
    print(f"Time taken to generate synthetic de data: {time.time() - start_time:.2f} seconds")
    print('############################################################################')

    start_time = time.time()
    synthetic_pt_df, pt_metadata_df = generate_synthetic_data(
        pt_df, metadata_base_path, metadata_type="pt", use_same_metadata_version=use_same_metadata_version
    )
    ...
    write_output_file(
        ...
        pa_df=synthetic_pa_df,                       # Synthetic  (pf_df)
        de_df=synthetic_de_df,                       # Synthetic details(de_df)
        pt_df=synthetic_pt_df,                       # Synthetic data page trailer (PT)    
        cd_df=cd_df,                                 # CD Records                  
        layout_dict={                                # Dictionary containing layouts for each data section
            'pa_layout': pa_file_layout,             # PA layout
            'de_layout': de_file_layout,             # DE layout (header1)
            'pt_layout': pt_file_layout              # PT layout            
        },
    ...
    start_time = time.time()
    evaluate_synthetic_data(pa_df, synthetic_pa_df, pa_metadata_df)
    evaluate_synthetic_data(de_df, synthetic_de_df, de_metadata_df)
    evaluate_synthetic_data(pt_df, synthetic_pt_df, pt_metadata_df)
