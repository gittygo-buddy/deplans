...
def write_output_file(output_file_path, synthetic_data, layout, synthetic_header, header_layout):
    ...
    with open(output_file_path, 'w') as outfile:
        ...
        for _, row in synthetic_data.iterrows():
            transformed_data = "".join(str(row[column_name]).ljust(length) if pd.notna(row[column_name]) 
                                       else ''.ljust(length) for column_name, length in zip(layout['Column_Name'], layout['Length']))
            outfile.write(transformed_data + '\n')
