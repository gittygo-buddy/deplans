...
def generate_synthetic_data(df, metadata_base_path, use_same_metadata_version=True):
    ...
    if latest_version and use_same_metadata_version:
        # Always use the same metadata version if specified
        ...
        # If no existing metadata found or if the flag is set to create new metadata
        print("No existing metadata found or creating new metadata.")
        metadata.detect_from_dataframe(df)
        save_new_metadata_version(metadata_base_path, metadata)  # Save newly detected metadata
...
# Step 7: Write the processed data to the output file, including the first and last rows
...
if __name__ == "__main__":
    main(use_same_metadata_version=False)  # Set to False to allow metadata updates
